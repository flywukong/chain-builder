...
export async function runReorgAnalysis(data) {
  const prompt = [
    ...
    "reorgsPerDay = 平均每天发生几次重组;orphansPerDayDedup = 平均每天有几个区块被回滚丢弃(同一个块只算一次);totalReorgs/totalOrphansDedup = 合计;peakDay = 次数最多的一天(只用于判断是否超过 10 次,平时不要输出);events = 具体事件,timeLocal 已是北京时间,approxBlockRange 是该事件所在小时对应的大致块高范围,nodes 是经历该事件的节点数;chainReorg24h = 最近 24 小时的次数。数据已剔除了单个节点自身抖动造成的假象,只统计多节点共同确认的真实重组。",
    "",
    ...
    "3. 事件列举(有才写):每条一行,细节写全——几月几号几点(timeLocal)、发生几次、回滚了几个块、块高范围(approxBlockRange,写「块高约 #A ~ #B」;为 null 就不写,严禁编造)、影响面(nodes 多=全网性,少=局部)。写法参考「7/6 20:00 发生 2 次重组,回滚 5 个块,块高约 #109,982,100 ~ #109,990,100,33 台节点经历,全网性」。",
    "细节就地给全,禁止写「可到 XX 页面/时间线查看」「点击分析按钮」这类引导语——读者不想再点别处。唯一例外:事件里没有出块方信息,严禁编造 validator 名字,想知道是谁被重组只能提一句「事件行的分析按钮可查出块方」。",
