...
function buildPrompt(data) {
  return [
    ...
    "",
    "输出格式(严格遵守):",
    "1. 第一行 [播报]:固定以 [播报] 开头、一行写完的结论口播——第一句是明确结论(如「主网运行正常,当前无需处理」),之后至多再加 1 句最有信息量的水位或事件;不用 markdown,60 字内。无论正常与否都必须有。",
    "2. 第二行:总体结论,只写 正常 / 需关注 / 告警 三选一(单独一行)。",
    "3. 之后固定输出四个小标题段落,每段用「**标题**」加粗开头另起一行,顺序严格为:**流量** → **Slash** → **Reorg** → **版本升级**。四段都必须有,即使该维度正常也要用一句话交代,不能省略某段。每段 1-3 句白话:",
    "   - **流量**:窗口内是否有大流量高峰(看 trafficEpisodesWindow / txpool24h)。有则逐个写具体时间(到分,如 7/18 16:00)、区块高度、峰值待处理笔数、区块 gas 是否打满、多久回落;无则一句「窗口内流量平稳」并给当前 pending 与峰值作参考。",
    "   - **Slash**:窗口内 slash(看 slashWindow)。有则写被 slash 的 validator(标注「自营」或「外部」)、连续块数、替代出块者、时间;多个先给总数(N 块 / M 段)再列前几个显著的;无则「窗口内无 slash」。",
    "   - **Reorg**:窗口内链级 reorg(看 reorgWindow)。有则写次数,并逐个给时间、区块区间、回滚块数;无则「窗口内无链级 reorg」。日均 0-3 次 micro-reorg 属正常,只客观陈述不要渲染成风险。",
    "   - **版本升级**:全网 geth 版本(看 versionUpgrade)。写当前最新版本 latest;若 laggardCount>0,给出落后节点数并列出名单(自营落后的优先点名并建议升级);全部最新则「全网已升级至 vX.Y.Z」。",
    "",
