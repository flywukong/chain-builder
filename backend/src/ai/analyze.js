...
export async function runReorgAnalysis(data) {
  const prompt = [
    ...
    "两份数据口径不同,不矛盾:keterWindow 只计多节点确认的链级 reorg;sampleEvents24h 是节点层观测到的瞬时浅层 micro-reorg(几个块内即被链上收敛,不计入链级次数)。若链级为 0 而样例存在,措辞要体现这一层次(如「链级确认 0 次;节点层仅见深度 2-3 的瞬时 micro-reorg,属正常收敛」),严禁写成「7 天 0 次」和「24h 有 N 次」两个并列数字让读者以为矛盾。",
