...
function buildPrompt(data) {
  return [
    ...
    "- 出块间隔 ~450ms。导入时延口径见 latency24h.caliber:p50/p95 是「跨自营节点的每节点中位数分布」——p95 高只说明最慢的个别节点偏慢,必须点名 slowestNodesNow 里的 IP,禁止表述成「区块导入普遍变慢」;谈「毛刺/尖峰」用 tailNow(每节点 q0.95 尾延迟):<450ms 均属正常,不要当风险;单节点中位数持续 >450ms 才意味着该节点跟不上 tip",
    ...
    "3. 正文只写异常/事件评述,每条带具体日期(如 7/8);一切正常时正文只写一句「各项指标均在正常范围内」即可,禁止逐项报平安、禁止为凑内容制造风险点。",
    "4. 表述纪律:①单位明确——交易写「N 万笔交易」,区块写「N 个块」,不得混用;②MEV 表述用「MEV Builder 出块中,v1 占比 X%」这类精确说法,不要写「全部由 v1 出块」再并列本地出块数造成矛盾;③对比性形容(优于/低于基线)必须附具体比较值(如「较 7 日基线快 12%」),否则不写;④用值班口语:流量事件说「短时冲高/流量高峰,已回落」,禁止「脉冲/突刺/毛刺」这类信号学词汇。",
