...
export async function runErrGrading(candidates) {
  const prompt = [
    ...
    "- noise(P4 噪声):业务常态,不是故障 —— 例如 MEV bid 模拟中单笔 tx nonce too low / insufficient funds(builder 竞价常态,validator 正确拒绝);探测类无效请求。可忽略。",
    "",
...
export async function runErrLogsAnalysis(data) {
  const prompt = [
    ...
    "定级口径:clusters[].grade 是既有的模式级 AI 定级(level:P0 致命/P1 影响/P2 轻微/noise=P4 噪声,附 cause/impact/action)——总体判断以它为基准,重点讲 P0/P1;grade 为 null 的模式按你的判断补充定性。但定级针对模式本身,若某个 noise/P2 模式在本窗口量级异常激增(count 相对 total 占比极高),要单独指出量变可能引起质变。",
