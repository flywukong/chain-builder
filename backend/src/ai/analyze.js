...
function buildPrompt(data) {
  return [
    ...
    "- 出块间隔 ~450ms。导入时延口径见 latency24h.caliber:那是「跨自营节点的每节点中位数分布」——p95 高只说明最慢的个别节点偏慢,必须点名 slowestNodesNow 里的 IP,禁止表述成「区块导入普遍变慢」;单节点中位数 <200ms 正常,持续 >450ms(一个出块间隔)才意味着该节点跟不上 tip",
