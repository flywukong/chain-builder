...
export async function runReorgAnalysis(data) {
  const prompt = [
    ...
    "reorgsPerDay = 平均每天发生几次重组;orphansPerDayDedup = 平均每天有几个区块被回滚丢弃(同一个块只算一次);totalReorgs/totalOrphansDedup = 合计;peakDay = 次数最多的一天(只用于判断是否超过 10 次,平时不要输出);events = 具体事件,timeLocal 已是北京时间;chainReorg24h = 最近 24 小时的次数。数据已剔除了单个节点自身抖动造成的假象,只统计多节点共同确认的真实重组。",
    "",
    ...
    "3. 事件列举(有才写):每条一行——几月几号几点(timeLocal)、发生几次、回滚了几个块、区块高度大约在哪(events 里没有块高就不写,严禁编造);写法参考「7/15 21:00 发生 1 次重组,回滚 5 个块」。事件里没有出块方信息,严禁编造 validator 名字;想知道影响到谁,提示用户在 Reorg 面板点击该事件做取证。",
