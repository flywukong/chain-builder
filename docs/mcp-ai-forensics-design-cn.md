...
> 2026-07-13 实测修正(stdio tools/list,45 个工具):**没有 `get_transaction_receipt`**(README 有误);receipt 类查证改用 `get_transaction`。场景 1/2/3/5/6 已落地:`backend/mcp/bnbchain.json` + `spawnClaude(prompt,{mcp:true})` + 12 工具只读白名单,写类工具实测存在(transfer/approve/create_erc20/confirm_transfer/gnfd 写类)均未放行。

| bnbchain-mcp 工具 | 场景 | 白名单 |
|---|---|---|
| `get_block_by_number` / `get_block_by_hash` / `get_latest_block` | 1/2/3/4/7 | ✅ |
| `get_transaction` | 1/2/5/6 | ✅ |
